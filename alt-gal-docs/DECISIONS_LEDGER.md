...
D-217: `jsdom@30` is a devDependency (D-073 requires the entry). §11.2's Stream 1 isolation test mounts TokenPanel under Vitest + Testing Library, and Vitest 4 ships no DOM environment. Root `vitest.config.mts` runs jsdom for every suite (the persisted stores need localStorage) and `vitest.setup.ts` supplies an in-memory Storage, because Node 25's jsdom hands back a `localStorage` with no methods. The config also sets `oxc.jsx.runtime = 'automatic'`: tsconfig's `jsx: 'preserve'` is for the Next compiler, which the test runner does not have.
D-218: Amends D-162: `text-muted` sits at `l=54`, not 58. Measured, not estimated — at 58 the pair text-muted/background is 2.76:1 for a green primary and 2.92:1 for an orange one, failing D-163's ≥3:1 (Turn 6's claim of "≈3.4:1 for a neutral hue" is arithmetically wrong; a neutral l=58 on l=98 is 2.91:1). At 54 all four D-163 primaries × four strategies clear 3:1 (3.12–3.65). Every other D-162 value is unchanged.
D-219: Amends D-046's selection rule, not its candidate pair: `onColor(base)` returns whichever of `hsl(h,15%,10%)` and `hsl(h,10%,98%)` contrasts more against the base, instead of switching on `base.l > 60`. Lightness is not luminance — the lightness rule handed white text to a green at l=45 (2.2:1), an amber at l=50 (2.04:1), and a red at l=60 (3.6:1). Contrast-max fixes those to 7.46 / 8.07 / 4.71 and is never worse. `--color-on-*` values stay in the same family, so nothing in Stream 2 changes. Known residual: `hsl(220, 90%, 56%)` reaches only 4.37:1 — its ceiling against ANY color is 4.59:1 — so D-163's on-color assertion is asserted for the other three primaries and the audit reports this one (D-165 check 2).
D-220: Amends D-120's implementation, not its contract: `fontStack(family)` returns `var(--font-inter), system-ui, sans-serif`, reaching next/font's renamed face through the CSS variable `fontLoader.ts` declares rather than through `FONTS[family].style.fontFamily`. `next/font/google` resolves to an empty module outside the Next compiler, and `utils/fonts` is imported by tokenToCss, the stores, and every test. This is also what D-142's `--studio-font: var(--font-geist)` already assumes. `publicFontStack` carries the D-121 export form.
D-221: `UNSET_COLOR` (D-109) is added to `src/utils/defaults.ts`, where Turn 4 §2.3 puts it and where tokenToCss, the accessibility auditor, and the canvas all import it from. The seed omitted it; this is the omission, not a new decision.
D-222: `src/components/tokens/PrimaryColorForm.tsx` is added to the eslint `PHASE_STORE_ALLOWED` list (D-197). The one declarative tool lives in the token panel by D-029, but `phase` is a required field of the ToolResult envelope D-030 makes it emit. One file, not the directory.
