...
D-218: Stream 2 adds devDependencies `jsdom` and `axe-core` and creates `vitest.config.ts` (jsdom environment, `@` alias, `src/**/__tests__/**/*.test.{ts,tsx}`). Required by Turn 3 §3.7 tests 2 and 3, which render all sixteen components and run axe-core over them; the seed had no DOM environment and no axe. Amends D-073's "no stream adds a dependency" for these two dev-only packages; the runtime dependency list in Turn 8 §17 (D-202) is unchanged.
