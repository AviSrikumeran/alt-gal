...
describe('every tool, valid and invalid input', () => {
  ...
  it('narrates the phase change in the envelope when one happens (D-006)', async () => {
    ...
    useComponentStore.getState().reset();
    // Phase 0 is "nothing defined" (D-006): with Stream 1's real token store now merged,
    // tokens set by earlier cases in this file keep computePhase() at 2 unless they go too.
