...
// Reads library.css off disk through import.meta.url; the consolidated jsdom config
// (D-225) leaves import.meta.url as a non-file URL, so this suite keeps Stream 5's
// node environment (its own config set `environment: 'node'` repo-wide).
